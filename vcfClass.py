...
class VCF:
		...
		*assumes one sample per vcf*
		...
		for mut in otherVCF:
			try:
				...
				if self._hashedmuts[mut.chrom][mut.pos] is None:
	...
	def addMut(self, newMut):
		raise NotImplementedError('addMut not implemented yet')
		...
		if len(bamfiles) == 0:
			...
			for sample, bamfile in bamfiles.items():
				if sample not in self.samples:
					raise ValueError(f'Sample {sample} not in VCF.')
			self.bamfiles.update(bamfiles)
		else:
			raise AttributeError('Unable to associate bamfiles with samples.')
	...
	def get_synonymous(self):
		return [mutation for mutation in self.mutations if mutation.AAtype == 'Synonymous']

	def get_nonsynonymous(self):
		return [mutation for mutation in self.mutations if mutation.AAtype == 'Nonsynonymous']
	...
	def to_numpy(self, referenceFile=None, cutoff_freq=0):
		...
		readCtsDF = pd.pivot_table(RDdf[['sampleID', 'inConcatPos', 'ref_nuc', 'RD']], columns='ref_nuc',
								   values='RD', index=('sampleID', 'inConcatPos'))
		...
		readCtsDF.update(pd.pivot_table(df[['sampleID', 'inConcatPos', 'alt_nuc', 'AD']], columns='alt_nuc',
										values='AD', index=('sampleID', 'inConcatPos')))
...
class MutCall:
	...
	def apply_pos_filter(self, bam, sample, cutoff=0.05, removeFails=True, snp_frequency_cutoff=0.01,
		...
		soft_clipped_positions = [read.alignment.get_reference_positions().index(pos)
								  if pos in read.alignment.get_reference_positions()
								  else 0
		...
		if sig_diff_pos_from_major_allele:
			...
			if (freq > snp_frequency_cutoff) and (freq < 1 - snp_frequency_cutoff):
	...
	def annotate(self, gene, adjust, dnaseq, AAseq):
		...
		# currently not calling indels, this is incomplete (cannot handle partial indels, multi-allelic sites)
	...
	def to_dict(self):
		...
		for samplename in self.samples:
			...
			result = {'sample': samplename, 'chrom': self.chrom, 'pos': self.pos,
			...
			if len(self.AApos) > 0:  # if annotated
				for gene, inGenePos, AAref, AApos, AAalt, AAstr, AAtype in zip(self.genes, self.genePOS, self.AArefs, self.AApos, self.AAalts, self.AAstrs, self.AAtypes):
					...
					geneSpecificResult.update({'gene': gene, 'inGenePos': inGenePos, 'refAA': AAref, 'AApos': AApos,
											   'altAA': AAalt, 'AAstr': AAstr, 'AAtype': AAtype})
	...
	def __str__(self):
		...
		return '\t'.join([self.chrom,
						  ...
						  self.filter,
						  self.info,
						  self.format]) + '\t' + '\t'.join(str(self.get(sample)) for sample in self.samples)
...
class SampleMut:
	def __init__(self, samplename, data, formatinfo):
		...
		self._properties = [self.GT, self.GQ, self.SDP, self.DP, self.RD, self.AD, self.freqstring, self.PVAL, self.RBQ,
							self.ABQ, self.RDF, self.RDR, self.ADF, self.ADR]
	...
	def to_dict(self):
		self.update()
		return {"GT": self.GT, "GQ": self.GQ, "SDP": self.SDP, "DP": self.DP, "RD": self.RD, "AD": self.AD, "FREQ": self.freq,
				"PVAL": self.PVAL, "RBQ": self.RBQ, "ABQ": self.ABQ, "RDF": self.RDF, "RDR": self.RDR, "ADF": self.ADF, "ADR": self.ADR}
